...
```
    ...
    好处：
        ...
        2.简化操作
        3.减少编译次数(只在第一次编译)
        4.减少和数据库的连接次数(一个存储过程的所有sql只连接一次数据库)
    关键字：procedure
    1、创建语法：
    ...
    参数模式：
        ...
        INOUT: 该参数既可以作为输入，也可以作为输出，也就是既需要传入值，又可以返回值
    ...
    2、如果存储过程体仅仅只有一句话，BEGIN END可以省略
        ...
        DELIMITER 结束标记
        例子：
        DELIMITER $  这是声明了一个$作为存储过程的结束符号，避免了使用;分号结束和sql语句的结束符号冲突
    ...
    #3.1 空参
        ...
        BEGIN
            ...
            VALUES('lisan','0000'),('lisi','1111'),('liwu','2222');
        END $
    ...
    好处：
        ...
        2.简化操作
        3.减少编译次数(只在第一次编译)
        4.减少和数据库的连接次数(一个函数的所有sql只连接一次数据库)
    函数和存储过程的区别：
        存储过程：可以有0或者多个返回，适合批量增删改
        函数：有且仅有1个返回，适合处理数据后返回一个结果，查询一个值
    ...
    1、创建语法：
        ...
        #1.1 参数列表：参数列表包含两个部分
            参数名 参数类型
        #1.2 函数体：肯定会有return语句
        #1.3 使用DELIMITER语句结束标记
